    ...
    if yara is not None:
        ...
        rules = yara.compile(
            filepaths={
                ...
                if file_name.lower().endswith(".yar") or file_name.lower().endswith(".yara")
            })
        ...
        logger.info("[Validation module] - Skipping YARA validation as YARA library is not installed")    
...
def check_file_size_allowed(file, upload_config):
    ...
    if upload_config["file_size_limit"] is not None:
        file_size_ok = (file.basic_information.size / 1000 <=
                        upload_config["file_size_limit"])
    else:
        file_size_ok = True
...
def check_request_header_mime(file, upload_config):
    ...
    if not mime_whitelist_result:
        file.block = True
        logger.warning(f"[Validation module] - Content-Type not whitelisted")
...
def check_signature_and_request_mime_match_file_extensions(file):
    ...
    for single_file_extension in file.detection_results.extensions:
        file_extension_mime = mimetypes.guess_type(
            "name." + single_file_extension)[0]
        ...
        mime_type_matching = (
            file.basic_information.content_type in possible_mime_types and
            file.detection_results.signature_mime in possible_mime_types)
    ...
    if not all_extensions_match:
        file.block = True
        logger.warning(f"[Validation module] - Extension MIME does not match")
...
def check_file_signature(file, upload_config):
    ...
    if not mime_whitelist_result:
        file.block = True
        logger.warning(f"[Validation module] - Signature not whitelisted")
...
def check_filename_length(file, upload_config):
    ...
    if upload_config["filename_length_limit"] != None:
        length_ok = (len(file.basic_information.name) <=
                     upload_config["filename_length_limit"])
    else:
        length_ok = True
    ...
    if not length_ok:
        file.block = True
        logger.warning(f"[Validation module] - Filename length too long")
...
def check_filename_extensions(file, upload_config):
    ...
    for single_extension in file.detection_results.extensions:
        curr_file_extension_mime = mimetypes.guess_type(
            "name." + single_extension)[0]
        mime_whitelist_results.append(check_mime_against_whitelist(
            curr_file_extension_mime, upload_config))
    ...
    if not all_extensions_whitelisted:
        file.block = True
        logger.warning(f"[Validation module] - Extension not whitelisted")
...
def check_filename_for_null_byte_injections(file):
    ...
    for file_name_split in file.detection_results.filename_splits:
        null_byte_found_in_split = (
            "0x00" in file_name_split or "%00" in file_name_split or "\0" in file_name_split)
    ...
    if null_byte_found:
        file.block = True
        logger.warning(f"[Validation module] - Null byte injection found")
...
def guess_mime_type(file):
    ...
    guessing_scores = {mime_type: 0 for mime_type in list(
    ...
    sorted_guessing_scores = {k: v for k, v in sorted(
        guessing_scores.items(), key=lambda item: item[1]) if v > 0}
    logger.debug(
        f"[Validation module] - {pprint.pformat(sorted_guessing_scores)}")
    logger.debug(
        f"[Validation module] - {total_points_overall=} - {total_points_given=}")

    guessed_mime_type = max(guessing_scores.items(),
...
def validate_file(file, upload_config):
    ...
    main_file_extension = file.detection_results.extensions[0]
    main_extension_mime_type = mimetypes.guess_type(
        "name." + main_file_extension)[0]
