...
def permission_check_required(model) -> bool:
    '''Determine whether permission check is required for the specified model
        by checking whether it has `acas_rbac_permission_check` set

    Args:
        model: Django model class

    Returns:
        True if the model has `acas_rbac_permission_check=True`
    '''
    return getattr(model, "acas_rbac_permission_check", False)
...
def handle_pre_save(sender, instance=None, **kwargs): # pylint: disable=unused-argument
    ...
    Raises:
        RbacPermissionDenied: Raise if the current loggined user doesn't have the required 
    ...
    if instance and instance.pk:
        ...
        if getattr(settings, "ACAS_RBAC_OBJECT_CONTROL", True):
        ...
        if not user.has_perm(perms['update'], instance_to_check):
            ...
            if getattr(settings, "ACAS_RBAC_BLOCK", True):
...
def handle_pre_delete(sender, instance, **kwargs): # pylint: disable=unused-argument
    ...
    if not user.has_perm(perms['delete'], instance_to_check):
        ...
        if getattr(settings, "ACAS_RBAC_BLOCK", True):
            raise RbacPermissionDenied(
                f"user {user} not allowed to delete {instance}")

@receiver(post_read)
@with_sudo
def handle_post_read(sender, queryset, **kwargs): # pylint: disable=unused-argument
    """Handles the acas extended `post_read` signal. `post_read` signal will be sent after user
    ...
    if not getattr(settings, "ACAS_RBAC_OBJECT_CONTROL", True):
        ...
        if getattr(settings, "ACAS_RBAC_BLOCK", True):
            # pylint: disable-next=protected-access
            raise RbacPermissionDenied(f"user {user} not allowed to read {queryset._result_cache}")
        return
...
def handle_pre_update(sender, queryset, **kwargs): # pylint: disable=unused-argument
    """Handles the acas extended `pre_update` signal. `pre_update` signal will be sent before
