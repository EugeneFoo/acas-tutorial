    ...
    def __init__(self, file):
        ...
        if file == '':
            ...
            try:
                ...
                logger.error('Error accessing URL %s' % file)
            ...
            try:
                ...
                logger.error('Error opening file %s' % file)
                logger.info(sys.exc_info()[1])
                sys.exit()
        else:
            try:
...
def UpdateWords(word, wordExact, slash, words, hexcode, allNames, lastName, insideStream, oEntropy, disarmed_buffer=None):
    if word != '':
        ...
        if disarmed_buffer is not None:
            if slash == '/' and '/' + word in ('/JS', '/JavaScript', '/AA', '/OpenAction', '/JBIG2Decode', '/RichMedia', '/Launch'):
                key_word_out = HexcodeName2String(SwapName(wordExact))
            else:
                key_word_out = HexcodeName2String(wordExact)
            disarmed_buffer.write(C2BIP3(key_word_out))
...
def PDFiD(file, allNames=False, extraData=False, disarm=False, force=False, disarmed_filebuffers=None, filebuffer=None):
            ...
            if charUpper >= 'A' and charUpper <= 'Z' or charUpper >= '0' and charUpper <= '9':
                ...
                if d1 != None:
                    ...
                    if d2 != None and (chr(d1) >= '0' and chr(d1) <= '9' or chr(d1).upper() >= 'A' and chr(d1).upper() <= 'F') and (chr(d2) >= '0' and chr(d2) <= '9' or chr(d2).upper() >= 'A' and chr(d2).upper() <= 'F'):
                        ...
                        (word, wordExact, hexcode, lastName, insideStream) = UpdateWords(word, wordExact, slash, words, hexcode, allNames, lastName, insideStream, oEntropy, disarmed_pdf_buffer)
                        if disarm:
...
def Main():
    ...
    if len(args) == 0:
        if options.disarm:
            logger.info('Option disarm not supported with stdin')
            options.disarm = False
        if options.scan:
            logger.info('Option scan not supported with stdin')
            options.scan = False
        filenames = ['']
    else:
        try:
            ...
            if oExpandFilenameArguments.warning:
                logger.info(oExpandFilenameArguments.message)
        except Exception as e:
            logger.info(e)
            return
